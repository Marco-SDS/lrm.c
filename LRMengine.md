...
| **16** | Background removal in C | Vendored U2Net (xuebinqin/U-2-Net, Apache-2.0 — the model rembg uses by default) in `lrm/lrm_u2net.c`; added conv2d dilation + maxpool2d + bilinear upsample + relu/sigmoid kernels. `lrmc infer --remove-bg` segments the foreground and feeds the mask as alpha to the existing preprocess | C forward parity vs PyTorch: max\|err\|=2.6e-5, IoU@0.5=1.0 (`make test-u2net`) | 3–6 wk | ✅ |
